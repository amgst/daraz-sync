...
if (isProduction) {
  // Behind a platform proxy (Vercel, Railway, ...) - needed for req.secure /
  // req.ip to reflect the real client instead of the proxy hop.
...
// and the SPA fallback are served directly from the deployment's output
// directory, so requests never reach this branch.
if (isProduction) {
